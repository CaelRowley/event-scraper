...
`.github/workflows/scrape.yml` runs once daily (03:00 UTC, full mode), publishes
the export window to Cloudflare R2, and fails loudly when sources break or yield
nothing. The exported JSON is **not** committed — the bucket is its only
