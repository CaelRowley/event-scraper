...
`.github/workflows/scrape.yml` runs once daily (03:00 UTC, full mode), syncs the
export window into D1, and fails loudly when sources break or yield nothing. The
exported JSON is **not** committed — D1 is its only consumer, so `public/berlin/`
is gitignored and regenerated by any run. The workflow's only commit is a
...
makes the D1 sync upsert-only so a partial scrape can't prune real rows; `0` or
unset means no cap — a normal full scrape with a pruning D1 sync.
